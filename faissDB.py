...
try:
    ...
    print("sentence-transformers not available. Debug the issue bro!!")
...
class RAGVectorStore:
    ...
    def process_with_llm(
        ...
        model_id: str = "qwen2.5:7b") -> str:
        """
        Process retrieved context with an LLM to generate an answer, summary, or analysis using Ollama.
        ...
        Args:
            ...
            model_id: The model ID to use (defaults to "qwen2.5:7b")

        Returns:
            The LLM's response
        """
        try:
            # Define the prompt template based on the task type
            if task_type.lower() == "summarize":
                prompt_template = ChatPromptTemplate.from_template(
                    "You are an AI assistant specialized in summarizing information. "
                    "Summarize the following context clearly and concisely, highlighting the most important points. "
                    "Focus on accuracy and clarity.\n\nContext:\n{context}"
                )
            elif task_type.lower() == "analyze":
                prompt_template = ChatPromptTemplate.from_template(
                    "You are an AI assistant specialized in deep analysis. "
                    "Analyze the following context thoroughly, identifying key insights, patterns, implications, and connections. "
            ...
            chat_model = Ollama(
                model=model_id,  # Use the provided model_id
            ...
            rag_chain = RunnableParallel(
                {"context": lambda x: retrieved_context, "question": RunnablePassthrough()}
